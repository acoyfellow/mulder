...
import { createWebMcpCompanion } from "./companion";
...
import { generateTools } from "./openapi";
...
const companion = createWebMcpCompanion({
  ...
  renderPage: () => new Response(fixturePage(), { headers: { "content-type": "text/html; charset=utf-8", "content-security-policy": selfScriptPolicy, "cache-control": "no-store" } }),
  dispatch: (request) => fixtureApi(request),
});
let custodySession = "";
let identitySession = "";
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const companionResponse = await companion.handle(request);
