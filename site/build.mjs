...
curl -O "$MULDER_SITE/downloads/acoyfellow-mulder-0.1.0.tgz"
...
const home = `<div class="shell hero"><div class="eyebrow">Tools for the website you already run</div><h1>Your website already knows what it can do. <span>Let browser agents use it.</span></h1><p class="hero-copy">Mulder lets a browser agent use selected read-only parts of your API. Add Mulder to your existing Worker. Your API still handles each request.</p><div class="actions"><a class="button" href="/docs/quickstart/">Build your first tool</a><a class="button secondary" href="#demo">Watch the real call</a></div><div class="proof-line"><span>Existing API stays unchanged</span><span>You choose each read-only tool</span><span>API keys stay on your server</span></div><div class="demo-frame" id="demo"><div class="demo-bar"><i></i><i></i><i></i><span>Product overview. The Chrome proof is checked separately.</span></div><video controls muted playsinline preload="metadata" poster="/demo-poster.png" aria-label="Mulder product overview"><source src="/mulder-demo.mp4" type="video/mp4"></video></div></div>
<section class="section section-rule"><div class="shell"><div class="section-head"><h2>Add Mulder to your existing Worker.</h2><p>Mulder adds tool routes next to the website and API routes you already run. A supported browser finds the tools when a person opens the page.</p></div><div class="flow-compare"><div class="compare"><div class="compare-label">SEPARATE SERVICE</div>${diagram("OpenAPI -> build an MCP server\n-> deploy another service\n-> set up each agent")}</div><div class="compare good"><div class="compare-label">MULDER</div>${diagram("Existing Worker + Mulder\n-> choose safe GET operations\n-> browser finds the tools")}</div></div></div></section>
...
const reference = docsPage("/docs/reference/", "Two functions connect Mulder to your Worker.", "Use these functions to add the page and tool routes.", `<h2 class="content-title">API reference</h2><h2>createWebMcpCompanion</h2>${code(`createWebMcpCompanion(options: {
  document: OpenApiDocument;
...
const pageDefinitions = [
  ["/", "Browser tools for your existing API", "Add selected read-only browser tools to your existing Worker.", home],
...
await cp(tarball, join(out, "downloads", "acoyfellow-mulder-0.1.0.tgz"));
